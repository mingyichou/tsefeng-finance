...
def cashflow_income_rows(clinic: str, pl) -> list[tuple[str, int]]:
    if clinic == "澤豐":
        return [
            ("玉山逐筆入帳", pl.esun_inflow_total),
            ("x6 豐沛金流（澤沛→澤豐）", pl.x6_fengpei_settle),
            ("x8 澤豐現金入帳", pl.x8_zefeng_cash_revenue),
            ("x10 手 KEY 收入", pl.x10_income_total),
        ]
    return [
        ("玉山逐筆入帳", pl.esun_inflow_total),
...
def cashflow_expense_rows(clinic: str, pl) -> list[tuple[str, int]]:
    if clinic == "澤豐":
        return [
            ("玉山逐筆出帳", pl.esun_outflow_total),
            ("x3 澤豐現金支出", pl.x3_zefeng_cash_expense),
            ("x9 謝松坊薪資", pl.x9_offsite_staff_pay),
            ("x10 手 KEY 支出", pl.x10_expense_total),
            ("x12 澤豐合約支出", pl.x12_zefeng_contract_expense),
            ("x13 周院長薪資（兩院總和）", pl.x13_zhou_doctor_salary),
        ]
    return [
        ("玉山逐筆出帳", pl.esun_outflow_total),
        ("中信進出戶出帳", pl.ctbc_outflow_total),
        ("x10 手 KEY 支出", pl.x10_expense_total),
    ]
...
def _cat_table(header: str, rows: list[tuple[str, int]],
               total_label: str, total: int) -> str:
    body = "".join(
        f"<tr><td>{_html.escape(n)}</td>"
...
def _month_breakdown_table(month_labels: list[str], income: list[int],
                           expense: list[int], net: list[int]) -> str:
...
def build_cashflow_quarterly(sb, clinic: str, year: int, q: int,
    ...
    bad = [roc_label(m) for m in period if not cf.complete(m, clinic)]
    if bad:
        return None, _incomplete_note(
            bad, [iss for m in period if not cf.complete(m, clinic)
                  for iss in cf.issues(m, clinic)])

    pls = [cf.clinic_pl(m, clinic) for m in period]
    ...
    body += _month_breakdown_table([roc_label(m) for m in period], inc, exp, net)
    ...
    subtitle = (f"報表季度：{year} 第 {q} 季"
                f"（{roc_label(period[0])} ~ {roc_label(period[-1])}）")
    return _doc(title, subtitle, body, generated), None
...
def build_cashflow_yearly(sb, clinic: str, year: int,
    ...
    subtitle = (f"報表年度：{year} 年"
                f"（涵蓋 {roc_label(period[0])} ~ {roc_label(period[-1])}）")
    return _doc(title, subtitle, body, generated), None
...
def build_pl_monthly(sb, clinic: str, month: str,
    ...
    body = _kpis([
        ("G 總收入", pl.g_total_income, False),
        ("N 總支出A(含合約)", pl.n_total_expense_a, False),
        ("O 盈餘A", pl.o_profit_a, True),
    ])
    body += "<h2>損益摘要</h2>" + _cat_table(
        "項目", [
            ("G 總收入 = B+C+E+F", pl.g_total_income),
            ("H 薪資支出合計", pl.h_salary_total),
            ("I 現金支出", pl.i_cash_expense),
            ("K 合約支出", pl.k_contract),
            ("M 其餘支出", pl.m_other_expense),
        ], "N 總支出A(含合約)", pl.n_total_expense_a)

    # 3 個趨勢：收入 G / 支出 N / 盈餘 O — 近 6 有效月
    vms = list(reversed(
    ...
    body += svg_line_chart(labels, [Series("G 總收入", g, BLUE)])
    body += "<h2>月度支出趨勢（近 6 個有效月份）</h2>"
    body += svg_line_chart(labels, [Series("N 總支出A", nn, RED)])
    body += "<h2>月度盈餘趨勢（近 6 個有效月份）</h2>"
    body += svg_line_chart(labels, [Series("O 盈餘A", o, GREEN)])
...
def build_pl_yearly(sb, clinic: str, year: int,
    ...
    if not period:
        return None, _incomplete_note(
            [str(year)], [f"{year} 年尚無任何月份資料"])
    bad = [roc_label(m) for m in period if not pld.complete(m, clinic)]
    if bad:
        return None, _incomplete_note(
            bad, [f"{m} G 或 H 為 0，資料未完全" for m in bad])

    pls = [pld.clinic_pl(m, clinic) for m in period]
    ...
    body += "<h2>年度各月 G / N / O</h2>"
    body += _month_breakdown_table(
        [roc_label(m) for m in period],
